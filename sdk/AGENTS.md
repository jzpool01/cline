---
description: Development reference for the Tarogo SDK workspace.
globs: "*.ts,*.tsx,*.js,*.jsx,*.json,*.md"
alwaysApply: true
---

# Tarogo SDK — Development Reference
...
- `@tarogo/shared`: shared contracts, schemas, path helpers, hook engine, extension registry, low-level utilities
- `@tarogo/llms`: provider settings/config, model catalogs, provider manifests, gateway contracts, handler creation
- `@tarogo/agents`: stateless agent loop, tool orchestration, hook/extension runtime, event streaming
- `@tarogo/core`: stateful orchestration, session lifecycle, storage, config watching, plugin loading, default tools, telemetry. Exposes `@tarogo/core/hub` for discovery, the detached daemon entry, WebSocket clients, and session/UI client adapters, plus `@tarogo/core/hub/daemon-entry` for launching the shared daemon

### Dependency Direction

```mermaid
flowchart TD
  shared["@tarogo/shared"] --> llms["@tarogo/llms"] & agents["@tarogo/agents"] & core["@tarogo/core"]
  llms --> agents & core
  agents --> core
...
- model/provider schemas or handler behavior: `@tarogo/llms`
- stateless loop, tool orchestration, streaming, hook/extension runtime: `@tarogo/agents`
- session lifecycle, storage, config watching, default tools, plugin loading, telemetry, hub runtime services, hub discovery, hub daemon spawn, and session-oriented client helpers (`HubSessionClient`, `HubUIClient`, `connectToHub`): `@tarogo/core` (hub pieces live under `src/hub/`)
- remote-config schemas, managed instruction materialization, blob upload metadata, and OpenTelemetry config normalization: `@tarogo/shared/src/remote-config`
...
```sh
bun -F @tarogo/shared test
bun -F @tarogo/llms test
bun -F @tarogo/agents test
bun -F @tarogo/core test:unit
bun -F @tarogo/cli test:unit
```

If a focused test command fails with a missing `@tarogo/*` export or missing `dist/` file, build the relevant dependency package or run `bun run build:sdk`, then rerun the same test command. Treat that as a workspace setup issue, not as evidence of a source-code bug.
...
- For `@tarogo/llms` provider/model routing rules, follow [packages/llms/AGENTS.md](./packages/llms/AGENTS.md).
