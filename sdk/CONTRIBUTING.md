# Contributing to the Tarogo SDK
...
| Package | Owns |
|---------|------|
| `@tarogo/shared` | Contracts, schemas, path helpers, hook engine, extension registry |
| `@tarogo/llms` | Provider settings, model catalogs, manifests, handler creation |
| `@tarogo/agents` | Stateless agent loop, tool orchestration, hook/extension runtime |
| `@tarogo/core` | Stateful orchestration, session lifecycle, storage, config, telemetry, hub runtime services, hub discovery, detached daemon, and hub client adapters (`@tarogo/core/hub`, `@tarogo/core/hub/daemon-entry`) |
...
- `examples`: plugin, hook, and cron automation examples (customizations upon Tarogo SDK)
...
```sh
bun -F @tarogo/core build|test|typecheck
bun -F @tarogo/agents build|test|typecheck
```
...
The CLI build (`bun -F @tarogo/cli build`) bundles packages from their compiled `dist/`, not their TypeScript source. If you edit a package and then build the CLI without rebuilding the package first, the CLI binary will silently include the old package code. Always run `bun run build:sdk` (or the relevant `bun -F @tarogo/<pkg> build`) before building the CLI when testing changes end-to-end.
...
```sh
bun pm ls @tarogo/core @tarogo/agents @tarogo/llms
```
