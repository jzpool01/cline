# Sidecar Architecture — @tarogo/code

## Overview

The sidecar is a single Bun process that handles the desktop backend runtime directly.

It imports `@tarogo/core` directly and serves the Next.js frontend over HTTP + WebSocket.
...
```typescript
import { LocalRuntimeHost } from "@tarogo/core";

const sessionManager = await TarogoCore.create({
  backendMode: "hub",
  capabilities: {
...
```typescript
import { ProviderSettingsManager, listLocalProviders, ... } from "@tarogo/core";
...
```typescript
import { SqliteSessionStore, resolveSessionBackend } from "@tarogo/core";
...
```typescript
import { ensureHubServer, sendHubCommand } from "@tarogo/core";
