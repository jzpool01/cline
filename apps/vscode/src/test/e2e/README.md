...
This directory contains the end-to-end tests for the Tarogo VS Code extension using Playwright. These tests simulate user interactions with the extension in a real VS Code environment.
...
- **`fixtures/server/`** - Mock API server for testing Tarogo's backend interactions
...
e2e("Test description", async ({ sidebar, helper, page }) => {
  ...
  await inputbox.fill("Hello, Tarogo!")
...
- **`sidebar`** - Playwright Frame object for the Tarogo extension's sidebar
...
await addSelectedCodeToTarogoWebview(page)
```
...
2. **Playwright will open:**
   - A VS Code window with Tarogo extension loaded
...
- **VS Code Configuration:**
  ...
  - Extension development mode with Tarogo loaded
  - Temporary user data and extensions directories

- **Mock API Server:**
  - Runs on `http://localhost:7777`
  - Provides mock responses for Tarogo API calls
